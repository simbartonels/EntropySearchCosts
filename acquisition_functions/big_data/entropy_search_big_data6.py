...
class EntropySearchBigData6(EntropySearchBigData):
    def __init__(self, comp, vals, incumbent, gp, cost_gp=None):
        super(EntropySearchBigData6, self)._initialize(comp, vals, incumbent, gp, cost_gp)

    def _compute(self, kl, costs, candidate):
        '''
        Returns (exp[kl] - exp[currentKL]) / log(costs).
        Args:
        ...
        '''
        return (np.exp(kl) - np.exp(self._current_kl)) / np.log(costs)